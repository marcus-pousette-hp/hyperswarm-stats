...
test('swarm stream counters keep active stats during close update ordering', (t) => {
  const swarm = createFakeSwarm()
  const stats = new HyperswarmStats(swarm)
  const rawStream = createRawStreamStats()
  const conn = createFakeConnection(rawStream)
  ...
  swarm.on('update', () => assertStreamCounters(t, stats, rawStream))
...
test('missing optional stream counters stay zero after close', (t) => {
  const swarm = createFakeSwarm()
  const stats = new HyperswarmStats(swarm)
